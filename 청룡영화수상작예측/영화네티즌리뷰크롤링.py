...
class OpenDriverMovieSite():
    ...
    def ReviewsToCSV(self):
        df_all = pd.DataFrame(self.allReviews)
        df_all.to_csv('allreviews_1.csv', sep=',', encoding='euc-kr')

    def ReviewsIndexToCSV(self):
        title = []
        code = []
        num_total = []
        last_page = []
        for i in range(len(self.allReviewsIndex)):
            y = self.allReviewsIndex[i].split("//")
            t = y[1].split(":")
            c = y[2].split(":")
            n = y[3].split(":")
            l = y[4].split(":")
        ...
        allreview_index = pd.DataFrame(({'제목': title, '코드': code, '총리뷰수': num_total, '마지막페이지': last_page}))
        allreview_index.to_csv('allreview_index_1.csv', sep=',', encoding='euc-kr')
...
if __name__ == "__main__":
    ...
    time.sleep(1)
    opendrivermoviesite.ReviewsToCSV()
    opendrivermoviesite.ReviewsIndexToCSV()
